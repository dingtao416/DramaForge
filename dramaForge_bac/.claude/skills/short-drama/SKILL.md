...
最终剧本展示必须稳定组织为三层，任何导出、预览、交付都按这个顺序：

1. **原始创意**：直接展示用户输入的故事描述；这不是 AI 生成内容，不改写、不扩写、不总结。
2. **剧本摘要**：由 AI 生成，用于快速审核项目骨架，必须包含自定义集数、故事类型、目标受众、核心梗、一句话故事、人物小传、故事概览。
3. **分集剧本**：由 AI 生成，用于直接拍摄，先给分集目录，再给每集完整正文；每集正文不能退化成摘要。

写作时把用户输入作为第一层参考，只生成第二层和第三层。若用户要求一次性生成完整剧本，也必须在交付中保留三层展示顺序。
...
{主要角色小传。每个角色包含身份、视觉形象、核心标签、身份背景、成长经历、性格特点、角色关系、成长弧线。}

### 故事概览

{三幕结构概述，3-5句，含关键反转和终局}
