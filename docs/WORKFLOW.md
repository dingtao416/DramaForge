...
| **上传剧本** | 上传 `.docx` 文件（≤10万字）；角色/场景只从 `角色表/出场人物` 与 `场景/场景表` 等显式字段解析 | `POST /api/v2/projects/{id}/script/upload` |
...
| 输入 | 来源 |
|------|------|
| 角色名列表 | Step 1 生成，或从上传剧本显式角色字段提取 |
| 场景名列表 | Step 1 生成，或从上传剧本显式场景字段提取 |
